export const defaultCourses = [
  {
    title: 'Python Full Stack Internship',
    slug: 'python-full-stack',
    description: 'Master Python, Django, REST APIs, and full-stack development with real-world projects.',
    duration: '8 Weeks',
    price: 200,
    skills: ['Python', 'Django', 'REST API', 'PostgreSQL', 'Git'],
    mode: 'Remote',
  },
  {
    title: 'MERN Stack Internship',
    slug: 'mern-stack',
    description: 'Build modern web applications with MongoDB, Express, React, and Node.js.',
    duration: '8 Weeks',
    price: 200,
    skills: ['MongoDB', 'Express', 'React', 'Node.js', 'JWT'],
    mode: 'Remote',
  },
  {
    title: 'Java Full Stack Internship',
    slug: 'java-full-stack',
    description: 'Enterprise-grade Java development with Spring Boot and microservices.',
    duration: '8 Weeks',
    price: 200,
    skills: ['Java', 'Spring Boot', 'Hibernate', 'MySQL', 'Microservices'],
    mode: 'Remote',
  },
  {
    title: 'Data Science Internship',
    slug: 'data-science',
    description: 'Learn data analysis, visualization, and machine learning fundamentals.',
    duration: '8 Weeks',
    price: 200,
    skills: ['Python', 'Pandas', 'NumPy', 'Matplotlib', 'Scikit-learn'],
    mode: 'Remote',
  },
  {
    title: 'AI & Machine Learning Internship',
    slug: 'ai-machine-learning',
    description: 'Deep dive into neural networks, NLP, and AI model deployment.',
    duration: '8 Weeks',
    price: 200,
    skills: ['TensorFlow', 'PyTorch', 'NLP', 'Computer Vision', 'MLOps'],
    mode: 'Remote',
  },
  {
    title: 'Cyber Security Internship',
    slug: 'cyber-security',
    description: 'Ethical hacking, network security, and vulnerability assessment.',
    duration: '8 Weeks',
    price: 200,
    skills: ['Ethical Hacking', 'Network Security', 'Penetration Testing', 'SIEM'],
    mode: 'Remote',
  },
  {
    title: 'UI/UX Design Internship',
    slug: 'ui-ux-design',
    description: 'Design beautiful, user-centered digital experiences with industry tools.',
    duration: '8 Weeks',
    price: 200,
    skills: ['Figma', 'User Research', 'Wireframing', 'Prototyping', 'Design Systems'],
    mode: 'Remote',
  },
  {
    title: 'Web Development Internship',
    slug: 'web-development',
    description: 'Frontend fundamentals with HTML, CSS, JavaScript, and modern frameworks.',
    duration: '8 Weeks',
    price: 200,
    skills: ['HTML', 'CSS', 'JavaScript', 'React', 'Responsive Design'],
    mode: 'Remote',
  },
  {
    title: 'Embedded Systems Internship',
    slug: 'embedded-systems',
    description: 'Build microcontroller-based embedded applications with sensors, firmware, and real-time interfaces.',
    duration: '8 Weeks',
    price: 200,
    skills: ['Embedded C', 'Microcontrollers', 'Arduino', 'Sensors', 'RTOS'],
    mode: 'Remote',
  },
  {
    title: 'VLSI Design Internship',
    slug: 'vlsi-design',
    description: 'Learn digital logic, HDL design, simulation, and verification workflows for VLSI systems.',
    duration: '8 Weeks',
    price: 200,
    skills: ['Verilog', 'VHDL', 'Digital Design', 'Simulation', 'FPGA'],
    mode: 'Remote',
  },
  {
    title: 'IoT & Smart Systems Internship',
    slug: 'iot-smart-systems',
    description: 'Develop connected IoT solutions using sensors, embedded boards, cloud dashboards, and automation.',
    duration: '8 Weeks',
    price: 200,
    skills: ['IoT', 'ESP32', 'Sensors', 'MQTT', 'Cloud Dashboard'],
    mode: 'Remote',
  },
];
